...
export const MenuBackground: React.FC<MenuBackgroundProps> = ({ isFlickering }) => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            // 1. GENERATE PERSISTENT RADIAL TEXTURE (Once)
            ...
            // MASK SPRITE (Provides the alpha fade)
            ...
            const updateLayout = () => {
                if (!app.renderer || !systemLayer) return;
                
                // Suppress rendering while shifting
                ...
                // Reposition persistent mask sprite
                maskSprite.position.set(centerX, centerY);
                maskSprite.width = lightRadius * 2.5; // Wider for better horizontal bleed
                maskSprite.height = lightRadius * 2.5;

                scanline.clear();
                // 1. Technical Trail (Illuminates grid in a short strip following the line)
                const trailHeight = 60;
                for (let h = 0; h < trailHeight; h++) {
                    const alpha = (1 - (h / trailHeight)) * 0.06;
                    // Draw segments upwards from the lead line
                    scanline.rect(0, -h, width, 1).fill({ color: 0x00ffff, alpha });
                }

                // 2. High-Intensity Lead Line
            ...
            const ticker = (t: PIXI.Ticker) => {
                if (!app.renderer || !systemLayer.renderable) return;
