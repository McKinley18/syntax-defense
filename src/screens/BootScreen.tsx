...
const BootScreen: React.FC<BootScreenProps> = ({
  ...
  return (
    <div className="audio-splash ui-layer" onClick={() => {
      if (bootPhase >= 18 || skipIntro) {
        wakeAudioSystem();
      }
    }}>
      {/* PERSISTENT BOOT HEADER (Moved down and right for rounded screens) */}
      {!skipIntro && bootPhase > 0 && bootPhase < 18 && (
        <>
      ...
      <div className={`boot-container ${isDistorted ? 'distorted' : ''}`} style={{ fontFamily: 'monospace' }}>
        ...
        {/* ACTIVE BOOT SEQUENCE */}
        {bootPhase > 0 && bootPhase < 18 ? (
          <>
            {/* ACT 1: AUTHORIZATION (Clean) */}
            {bootPhase >= 1 && bootPhase < 5 && (
              <>
                <div style={{ color: '#fff', fontSize: '0.85rem', marginBottom: '12px' }}>
